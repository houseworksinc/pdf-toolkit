...
This module provides endpoints for generating PDFs from templates and zipping them
...
from app.workers.process_and_zip_worker import process_and_zip_task
...
process_and_zip_bp = Blueprint("process_and_zip", __name__)


@process_and_zip_bp.route("/api/v1/process-and-zip", methods=["POST"])
@require_jwt_token
def process_and_zip_route():
    """
    Process and zip multiple documents into a single ZIP archive.

    This endpoint can:
    1. Generate PDFs from templates (static or dynamic mode)
    2. Download existing documents from URLs
    3. Create a ZIP archive with all documents in the specified order
    ...
    for idx, doc in enumerate(documents):
        ...
        if doc_type == "url":
            ...
            prepared_doc["output_filename"] = doc.get("output_filename")
    ...
    # Queue the process and zip task with job_id
    task = invoke_task_with_priority(
        process_and_zip_task,
        priority=priority,
...
@process_and_zip_bp.route("/api/v1/process-and-zip/status", methods=["GET"])
@require_jwt_token
def get_process_and_zip_status():
    """
    Get the status of a process-and-zip job.
    ...
    # Check if job is process_and_zip type
    if job_info.get("job_type") != JobType.PROCESS_AND_ZIP:
        return jsonify(
            {
                "status": "Bad Request",
                "status_code": 400,
                "error": {
                    "message": f"Job is not a {JobType.PROCESS_AND_ZIP} job"
                },
            }
        ), 400
    ...
    if total > 0:
        # Progress: 0-90% for document processing, 90-100% for zipping
