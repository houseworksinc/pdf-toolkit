...
from app.workers.celery_worker import merge_pdfs_task
...
@merge_pdf_bp.route("/api/v1/merge-pdfs", methods=["POST"])
@require_jwt_token
def merge_pdfs_route():
    ...
    Supported formats:
    - PDFs, Images (png, jpg, jpeg, gif, bmp, tiff, svg)
    - Documents (doc, docx, odt, rtf, txt, epub, html)
    ...
    {
        "client_job_id": "1234",
        "document_urls": ["https://url1.com/doc.pdf", "https://url2.com/image.png"],
        "output_filename": "merged-output",  # optional
    ...
    # Queue the merge PDFs task with job_id
...
@merge_pdf_bp.route("/api/v1/merge-pdfs/status", methods=["GET"])
@require_jwt_token
def get_merge_pdfs_status():
    """
    Get the status of a merge PDFs job.
    ...
    # Check if job is merge type
    if job_info.get("job_type") != JobType.MERGE:
        return jsonify(
            {
                "status": "Bad Request",
                "error": {"message": f"Job is not a merge job"},
            }
        ), 400
