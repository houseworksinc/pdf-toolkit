"""Celery worker for PDF generation from templates (static mode)"""
...
def register_generate_pdf_task(celery_app, s3_client, s3_bucket):
    """Register the generate_pdf_task with the Celery app"""

    @celery_app.task(name="generate_pdf_task", bind=True)
    def generate_pdf_task(
        ...
        """
        Celery task to generate PDF and upload to S3 (Static mode - always requires template)

        Args:
            job_id: Job UUID (primary key)
            client_job_id: Client's unique job identifier
            template_url: URL to download the template from (required)
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Upload to S3 with custom filename if provided
                if output_filename:
